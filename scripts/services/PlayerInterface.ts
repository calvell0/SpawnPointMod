...
export default class PlayerInterface {
  ...
  static sendWelcomeMessage = (player: Player) => {
    // PlayerInterface.setTitle(event.player, "Welcome, Sack Chaser!");
    system.runTimeout(() => {
      player.sendMessage("Type §3!setspawn §fin chat to set a secondary spawn point." +
        " You'll be able to select which point to spawn at after dying. " +
        "Type §3!help §ffor a list of commands.");
    }, 100);
