...
export default class AnnoyingMobSpawner implements SecretEvent{
  ...
  run(event: PlayerInteractWithBlockAfterEvent): void {
    ...
    if (randomChance(0.33)){
      const rngDelay = Math.random() * AnnoyingMobSpawner.MAX_TICK_DELAY;
  ...
  executeAction(player: Player){
    ...
    const entityTypeToSpawn: string = "minecraft:" + pickRandomUnweighted<string>(AnnoyingMobSpawner.entityChoices);

    const newEntity: Entity = player.dimension.spawnEntity(entityTypeToSpawn, spawnLocation);
  }

  blockIsInList(block: Block):boolean {
    for (let blocktype of AnnoyingMobSpawner.blockInteractList){
